...
`CommandService` and `IndexerOperations` are application-scoped dependencies exposed through `IndexerNodeComponents`. Composition creates one operations instance and one command service, registers cleanup before delete initiation, registers remaining core and plugin handlers, and injects the same dependencies into admin paths. `DocumentStoreCommandHandlers.register(commandService, config)` is the canonical core registration helper; the command-unaware `create(config)` helper is removed because it cannot compose complete chained workflows. `IndexerOperations` is the indexer-entity lifecycle metadata layer. It is distinct from runtime `Indexer`, provisioning operations, `IndexerDocumentIndexResourceManager`, and `IndexerQueueResourceManager`; no general physical `IndexOperations` abstraction is introduced.
...
Core lifecycle transitions are exposed through an injected `IndexerOperations` component. Its first operation, `markDeleting(MarkIndexerDeletingRequest)`, owns expected-version validation, transitions to `DELETING` and `NON_ACTIVE`, publishes lifecycle invalidation, and returns the updated `IndexerRecord`. The layer may absorb other cohesive core lifecycle operations later, but this slice does not move unrelated behavior speculatively. `DeleteIndexerCommandHandler` and `AdminServiceImpl` share this operation before submitting cleanup. `IndexerOperations` does not own physical resource cleanup, command chaining, load behavior, or provisioning; those remain in their existing orchestration and provisioning components.

Delete initiation and cleanup are idempotent. `markDeleting(...)` returns an empty result for an absent indexer, returns the current record for an already-deleting indexer regardless of the original requested version, and otherwise requires the expected version before transitioning. `DeleteIndexerCommandHandler` submits `CleanupDeletingIndexerCommand(indexerId, expectedVersion)` using the returned current version and requires `CommandService`; cleanup cannot be silently omitted. Cleanup succeeds when the record is already absent, rejects records not marked `DELETING`, and treats a version mismatch as retryable stale state. Successful physical cleanup passes the same expected version to atomic metadata finalization.
...
The metadata-change notification is a fan-out wake-up for runtime nodes and local caches, not the source of truth. `IndexerRuntime` subscribes to indexer metadata changes, reloads the latest metadata indexer identified by the event, maps it to an `IndexerModel` for runtime transport, and reconciles local resources from that model. Target metadata changes use the same lifecycle event bus for cache invalidation, but do not drive runtime indexer reconciliation by themselves. Runtime construction can use the Verticle-backed constructor so active indexers deploy an `IndexerProcessorVerticle` while `IndexerRuntime` only tracks `Indexer` instances and never exposes Vert.x deployment ids. Production implementations should back `IndexerLifecycleEventBus` with a durable pub/sub topic. The in-memory implementation retains events and replays them to late subscribers for local tests.
