...
- `GatewayRestVerticle`: external gateway boundary. It is deployed separately from service-level REST APIs and is the planned owner for authentication, authorization, rate limits, external audit logging, and proxying selected public routes to REST APIs. It is OpenAPI-backed, runs gateway request hooks around each operation, and the first proxy slice forwards selected admin read traffic to the configured internal admin REST base URI.
